...
def SearchLicensePlate(chromeWebDriver, processingFile):
  ...
  if not plate or not state:
    return
  #//*[@id="cfx-select-input-1166372580852149"]  
  #/html/body/div[1]/div[1]/main/div/div[3]/div[2]/div/div[2]/div/div[1]/div[2]/div
  #/html/body/div[1]/div[1]/main/div/div[3]/div[2]/div/div[2]/div/div[1]/div[2]/select
  licenseInputXPath = '//input[@name="licensePlate"]'
  ...
  if str(responseText).__contains__("Bad recaptcha token") :
        ...
        ProxyManager.markProxyBad(proxyver)
        print(data)
        chromeWebDriver.close()
  else :
    if responseText['vinInfos'] :
        vin = list(responseText['vinInfos'].keys())[0]
