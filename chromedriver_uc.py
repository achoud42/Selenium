...
class ChromeWebDriver:
  ...
  #DRIVER_PATH = '/home/licensesearch/chromedrivers/v113/chromedriver'
  DRIVER_PATH = '/home/licensesearch/chromedrivers/v113/chromedriver'

  DEBUG_PATH = '/home/webdriver_debug/'
  ...
  def __init__(self, index, proxy, startindex):
    ...
    self.port = str(randint(1000, 500000))
    self.error = None
    self.usageCount = 0
    self.proxy = proxy
    ...
    #chromeOptions.binary_location = "/usr/bin/google-chrome"    #chrome binary location specified here
    ...
    chromeOptions.add_argument('--user-agent=Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/113.0.5672.63 Safari/537.36')
    ...
    chromeOptions.add_argument("--headless")
    print(self.proxy)
    chromeOptions.add_argument('--proxy-server=' + self.proxy)
    ...
    self.driver = uc.Chrome(options = chromeOptions,version_main=113,seleniumwire_options=options)
    #self.driver = webdriver.Chrome(service=Service(ChromeDriverManager().install()),seleniumwire_options=chromeOptions, desired_capabilities=capabilities)
    #self.driver = uc.Chrome(executable_path=self.DRIVER_PATH, chrome_options=chromeOptions,seleniumwire_options=options,desired_capabilities=capabilities)
    #self.driver = uc.Chrome(chrome_options=chromeOptions,seleniumwire_options=options)
    #self.driver = uc.Chrome(seleniumwire_options=options)
  ...
  def getXhrReponse_da(self, xhrSearchPath):
      out = None
      for request in self.driver.requests:
          if request.response:
             if xhrSearchPath in request.url:
                out = json.loads(request.response.body)
      return out

  def getXhrReponse4(self):
    out = None
    print('*************************************CAMCCECECECECEE')
    for request in self.driver.requests:
      try:
